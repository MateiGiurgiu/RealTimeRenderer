...
bool VoxelTerrain::SpaceshipHit(Vector3 const& position)
{
	if (position.y < 0.0f)
	{
		const Vector3 relativePos = Vector3(
			(position.x + static_cast<float>(X_MAX) * 0.25f) * 2,
			(-position.y + 0.5f) * 2,
			(position.z + static_cast<float>(Z_MAX) * 0.25) * 2
		);
		const unsigned int x = Clamp(static_cast<unsigned int>(relativePos.x), 0, X_MAX);
		const unsigned int y = Clamp(static_cast<unsigned int>(relativePos.y), 0, Y_MAX);
		const unsigned int z = Clamp(static_cast<unsigned int>(relativePos.z), 0, Z_MAX);
...
unsigned int VoxelTerrain::Clamp(unsigned int value, unsigned int const min, unsigned int const max) const
{
...
bool VoxelTerrain::InsideBounds(DirectX::SimpleMath::Vector3 const & const position) const
{
	...
	const bool bX = (x >= 0 && x < X_MAX);
	const bool bY = (y >= 0 && y < Y_MAX);
	const bool bZ = (z >= 0 && z < Z_MAX);
