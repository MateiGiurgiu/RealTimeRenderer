...
Camera::Camera(const DirectX::SimpleMath::Vector3& initialPos, const float screenWidth, const float screenHeight, const float fov, const float nearPlane, const float farPlane)
	: m_pos(initialPos), m_initialPos(initialPos), m_yaw(0.0f), m_pitch(0.0f), m_movementSpeed(3.0f), m_rotationSpeed(3.0f), m_delta(0.0f)
{
...
void Camera::Update(const float deltaTime)
{
	...
	{
		Vector3 pos = m_followTarget->GetPosition();
		Vector3 up = Vector3::Transform(Vector3(0, 1, 0), m_followTarget->GetOrientation());
		up.Normalize();
		pos.z -= 2.1f;
		pos += up * m_delta;
...
Matrix Camera::GetViewMatrix() const
{
	...
	{
		Vector3 lookAt = m_followTarget->GetPosition();
		Vector3 up = Vector3::Transform(Vector3(0, 1, 0), m_followTarget->GetOrientation());
		up.Normalize();
		lookAt += up * m_delta;
		return Matrix::CreateLookAt(m_pos, lookAt, Vector3::UnitY);
